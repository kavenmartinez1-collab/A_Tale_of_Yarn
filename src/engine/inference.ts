...
export interface InferenceSessionConfig {
  ...
  /** Existing GPU context to share (e.g., the game's device). When omitted a
   *  fresh device is requested — never pass omitted from a page that already
   *  holds a device. */
  gpu?: GPUContext;
}
...
export async function createInferenceSession(
  ...
  const gpu = sessionConfig.gpu ?? await initWebGPU();
